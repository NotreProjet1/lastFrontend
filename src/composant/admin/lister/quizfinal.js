...
const QuizDetailsTable = () => {
  const [quizzes, setQuizzes] = useState([]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:3000/quizFinal/tableAdmin');
    ...
    () => [
      {
        ...
        header: 'Instructeur qui met',
        size: 200,
      },
      {
        accessorKey: 'titre_quiz',
        header: 'Titre du Quiz',
        Cell: ({ cell }) => (
          <div style={{ display: 'flex', alignItems: 'center' }}>
            <FaLink style={{ marginRight: '8px' }} />
            <Link to={`/QuizfinalDetail/${cell.row.original.id_Q}`} style={{ color: '#007bff', textDecoration: 'none', borderBottom: '2px solid #007bff', transition: 'border-color 0.3s ease' }}>
              {cell.getValue()}
            </Link>
          </div>
        ),
      ...
      },
      {
        ...
        header: 'Participant qui apprend',
        size: 200,
      },
      {
        accessorKey: 'titre_formation',
        header: 'Formation Correspond',
        size: 200,
      },
    ],
    [],
...
export default QuizDetailsTable;
